...
describe("operations CLI helpers", () => {
  it("evaluates a machine-readable QA input file", async () => {
    const path = join(mkdtempSync(join(tmpdir(), "loopstack-qa-")), "qa.json");
    writeFileSync(path, JSON.stringify({ manifest: "valid", connections: "verified", storageContract: "verified", scenarios: "pass", approvals: "pass", idempotency: "pass", alerts: "pass", canary: "pass" }));
    expect((await evaluateQaFile(path)).verdict).toBe("pass");
  });
